...
export default function Login() {
  ...
  if (params.get('error') === 'oauth') toast.error('Google login failed. Try again.');
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50 dark:bg-gray-950">
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="card p-8 space-y-5">
          <button onClick={() => (window.location.href = `${import.meta.env.VITE_API_URL || ''}/api/auth/google`)} className="btn-secondary">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <InputField label="Email" type="email" placeholder="you@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} error={errors.email} />
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Forgot password?</Link>
            </div>
